...
@IBDesignable
open class Floaty: SoftViewBase {
    // MARK: - Properties
  ...
  //      mainColor = UIColor.colorScheme.dustyRose.cgColor
    ...
    @objc open func open() {
        ...
        circleLayer.backgroundColor = selectedColor != nil ? selectedColor?.cgColor : buttonColor.cgColor
    ...
    @objc override open func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        if closed == false {
            for item in items {
...
//    circleLayer.cornerRadius = size/2
    ...
    override open func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        if (object as? UIView) == superview && (keyPath == "frame" || keyPath == "bounds") {
            ...
            frame.origin.x = ((superview!.bounds.size.width - horizontalMargin) - paddingX) + scrollView.contentOffset.x
            frame.origin.y = ((superview!.bounds.size.height - verticalMargin) - paddingY) + scrollView.contentOffset.y
        }
    }
